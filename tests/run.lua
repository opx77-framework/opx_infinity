...
do
	...
	if why == nil then
		...
		-- tunable says now.
		--
		-- 2000ms is `TAGS.REFRESH_MS` in `config/admin.lua`, reached through the
		-- declared tunable. This used to read 500ms -- the FLOOR the call passes as
		-- its second argument -- and that was the harness lying: the host's
		-- `tunables.declare` stub answered the declaration block instead of a value
		-- proxy, so `OPX.Tune.Get` found a spec table where a number belongs and
		-- every `OPX.Tune.Number` in the resource fell through to its floor. The
		-- old assertion therefore passed whether the tunable worked or not, which
		-- is the one thing it existed to tell apart.
