...
local function answer(source, lines)
	...
	TriggerClientEvent(M.LINES, source, lines)
end
...
local function moduleLines()
	...
	-- The repeating work, under the modules that registered it: a module can be
	-- `started` and its sweep still be failing every pass, and that is invisible
	-- from the list above.
	local jobs = OPX.Scheduler.Report()
	if #jobs > 0 then
		lines[#lines + 1] = ''
		lines[#lines + 1] = ('%-28s %8s %s'):format('job', 'every', 'state')
		for index = 1, #jobs do lines[#lines + 1] = jobs[index] end
	end
	return lines
end
